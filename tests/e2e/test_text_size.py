...
def test_small_and_default_steps(authed_page: Page, base_url: str) -> None:
    page = authed_page
    page.goto(f"{base_url}/", wait_until="domcontentloaded")
    _open_settings(page)
    control = page.locator("#textSizeControl")
...
def test_large_at_320px_never_scrolls_sideways(
    authed_page: Page, base_url: str
) -> None:
    page = authed_page
    page.add_init_script(
        "try { localStorage.setItem('app-launcher.textsize', 'large'); } catch (e) {}"
    )
