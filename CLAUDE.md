...
- **Verification:** during a dev loop, run the e2e suite with the tray up — `.\scripts\run-e2e.ps1` (both projections) or `--browser chromium` for a faster pass. It runs `tests/e2e/test_smoke.py` plus a regression test per closed iPhone bite (cache hygiene, index revalidation, WS reconnect #28, paste button #29, pywinpty loopback hidden, Edge mirror window #20, WebKit viewport #31, terminal fling-scroll #23). Before declaring any webapp/launcher/session-host change done, run the full pre-ship gate `pwsh -File scripts/verify-before-ship.ps1` (boots its own disposable webapp + session-host — no tray needed). See README "Playwright smoke + regression tests" and "Verifying changes before ship" for detail. The non-browser suite alone is `pytest tests -m "not smoke" -v`.
