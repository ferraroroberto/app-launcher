...
**There is no safe launcher-side suppression, and we deliberately ship none.** The repaint is legitimate output and a faithful terminal must render it; the copy to drop would be the one *already in scrollback*, and xterm.js exposes no way to delete scrollback lines (`clear()` takes all of it, and `buffer` is read-only). Rewriting the byte stream server-side instead would mean second-guessing a live TUI from a partial model of its screen — losing real output or mangling a frame, unrecoverably, from a phone. The remaining honest levers are viewport height (the pan-not-reflow path the full-screen agents already take, #264/#268) and reading the conversation somewhere that is not xterm scrollback — the transcript view (#953) and Chat mode (#982), both of which reconstruct from the stream rather than from a scrolled viewport. **Lesson: when the duplicate is upstream and faithful, measure it and route around it — do not teach the terminal to lie.**
