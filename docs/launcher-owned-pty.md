...
### Every resize is a repaint someone keeps

Long Claude messages showed up two or three times in the phone's scrollback (issue #930). The duplication was in the raw PTY stream: each resize SIGWINCHes Claude Code into a full-viewport repaint (`ESC[H` then `N × ESC[2K ESC[1B`), and `ESC[2K` only erases the viewport — the copy already in scrollback survives, so the repaint *adds* one. The iOS keyboard sweep fires `visualViewport` events mid-animation and the phone forwarded every intermediate row count (`44 → 20 → 6 → 20 → 44`, including 1- and 6-row samples that are never a real viewport). The fix keeps xterm's local `fit()` immediate but settles the frame that reaches the PTY (~300 ms), floors it (`PTY_MIN_ROWS`/`PTY_MIN_COLS`, clamped again server-side), and makes `PtySession.resize()` a no-op for an unchanged size. **Lesson: a resize is not free for a PTY — the UI can track every frame of an animation, the agent should only hear about where it settled.** The full-screen agents sidestep it entirely by panning instead of reflowing (#264).
