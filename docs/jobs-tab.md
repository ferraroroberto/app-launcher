...
- **Failed spawns record why.** When `spawn_run_job_detached` raises `OSError`, the run is finalised on disk as `status="failed"` / `exit_code=-1` with the reason on the record — `spawn_error` for route fires, `chain_spawn_error` for chain fires — instead of stranding a `pending` record with no explanation. The route additionally answers `500 spawn failed: <reason>`. (Executor fires never reach this branch: they queue or run inline, they never spawn.)
