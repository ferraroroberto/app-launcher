...
| `idle-finished` | The turn ended clean — no pending tool_use of any kind found in the tail. The session doesn't actually need anyone; it's just Claude holding a workspace. | Claude's turn |
...
2. **Manual Start/Restart** — chat mode shows a status row (`chief: not running — Start`, or `chief: <status> — Restart` when one is alive, #617) for after a deliberate tray/session-host kill, or to cycle a stale chief mid-day. Restart calls `ensure?fresh=1` — a graceful stop-then-respawn, letting the #442 handover log carry context to the fresh session; it never restarts the session-host itself (that would kill every other PTY too, #615's territory). The same status row + Start button exists in the **Coding tab**'s Running-sessions card (#547) — `ensureChief()` is exported from `board.js` and called from `sessions.js`, so both tabs hit the identical endpoint. This used to be one of two respawn triggers alongside an unattended daily job (`chief-daily-respawn`, default 05:00); #616 retired that job once fleet-config#442/#449 shipped compact-and-continue (chief hands its own handover log back to itself on every session start) — an unattended daily restart would now discard a live batch's context instead of protecting it, so a restart is a deliberate operator action only, never a schedule.
3. **Resume** — outside the launcher's own triggers, Roberto sometimes reattaches the chief's underlying Claude Code conversation via the Coding tab's Resume flow (e.g. after a session-host restart killed its PTY). This never touches `ensure`, so the new PTY gets no `label` at spawn — see the self-heal note below.

**Label self-heal (#617).** `ensure`'s `label="chief"` at spawn, plus the legacy `name=="chief"` fallback, both miss a chief that reaches this state some other way (a manually typed `/chief`, or the Resume trigger above) — in both cases the fallback also reads the wrong field: the launcher's own session `name` is the *project* name (`"fleet-config"`) for a Resume-launched session, never `"chief"`, so that fallback looks like coverage but has never actually fired for this case. `_reconcile_chief_label` (`app/webapp/routers/board.py`) self-heals `label` read-time from either of two independent signals: `prompt_title` (#266's capture of the PTY's first *submitted* line — exact for a freshly typed `/chief`, but a Resume never resubmits it) or `shared_name` (fleet-config#302's hook-derived Claude Code session identity, joined by the same agent-aware claim walk every other cross-tab title uses — this one persists across a Resume into a brand-new PTY because it belongs to the *conversation*, not the process currently attached to it). Both are scoped to a live PTY cwd'd in fleet-config. This is what keeps `chief_ops.py`'s `chief-sid` lookup (fleet-config#443's escalation routing) and worker-cap count correct regardless of how the chief got started.
