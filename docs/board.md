...
Net cost per 5 s poll: one `os.stat` per session row, one ≤8 KB tail read for rows in a waiting status whose mtime moved (the #309 activity check), plus one more ≤8 KB tail read, unconditional for every row in a waiting status, for the #464/#576 pending-dispatch check.
...
3. **Manual Start/Restart** — chat mode shows a status row (`chief: not running — Start`, or `chief: <status> — Restart` when one is alive, #617) for after a deliberate tray/session-host kill, or to cycle a stale chief mid-day. Restart calls `ensure?fresh=1` — the same graceful stop-then-respawn as the daily job, letting the #442 handover log carry context to the fresh session; it never restarts the session-host itself (that would kill every other PTY too, #615's territory). The same status row + Start button exists in the **Coding tab**'s Running-sessions card (#547) — `ensureChief()` is exported from `board.js` and called from `sessions.js`, so both tabs hit the identical endpoint.
4. **Resume** — outside the launcher's own triggers, Roberto sometimes reattaches the chief's underlying Claude Code conversation via the Coding tab's Resume flow (e.g. after a session-host restart killed its PTY). This never touches `ensure`, so the new PTY gets no `label` at spawn — see the self-heal note below.
