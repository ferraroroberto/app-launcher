...
> **🛑 Retention decision (2026-06-22, issue #258 — do not delete yet).** Although the viability gate above is answered, this spike — its route (`/spike/voice-loop`), the three `spike-voice-loop.*` static files, the off-device test, and this doc — is **intentionally kept** until the **kanban / board view and the fleet orchestrator (#245) voice mode land**. Until then it stays the **live reference** for the v1 follow-ups below: wiring the real orchestrator narration into the loop in place of the mock, driving narration from summarized board state, and adding a real "conversation mode" entry point. The `/codebase-audit` stale-finding ([#258](../../../issues/258)) is **deferred on this basis, not unaddressed** — delete the set only once #245 + the board view have shipped and no longer need it. (As long as it still works, keep it.)
