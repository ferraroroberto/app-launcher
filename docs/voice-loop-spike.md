...
> **Status: retained — expiry condition in [#1054](../../../issues/1054).** This spike — its route (`/spike/voice-loop`), the three `spike-voice-loop.*` static files, the off-device test, and this doc — **is deleted once a hands-free conversation mode exists outside `spike-voice-loop.*`**: when **re-arm** (narration end returns to listening with no user tap), **barge-in** (sustained user speech cuts playback) and a **conversation-mode entry point** the user can actually open all live in shipped app code. The condition names the artifact, not a ticket, because naming a ticket is what made the old wording misreadable ([#1004](../../../issues/1004)): #245 shipped with the continuous hands-free loop *explicitly deferred*, so its closing is evidence against the condition, not for it. Also **not** satisfying it: dictation (`voice.js`, shipped, speech → text into a field), read-aloud (`terminal-readaloud.js`, shipped, one-way narration), and the Board view (#164/#302) — each was already discounted when the condition was last narrowed. Grep-level check anyone can run: `barge-in` appears today only inside the spike's own footprint — the three `spike-voice-loop.*` files, this doc, `tests/e2e/test_spike_voice_loop.py`, `tests/test_voice_loop_spike_retention.py`, and the `/spike/voice-loop` route docstring in `app/webapp/routers/misc.py`. When it appears in shipped app code outside that footprint — not in a second prototype — this set has a replacement and goes. (Do **not** grep `re-arm`: the word is already used for the tray watchdog, the e2e gate's webapp, and keyboard-modifier state, so it cannot discriminate.) That check is executable — `tests/test_voice_loop_spike_retention.py` runs it in the non-browser suite, so the next sweep gets its answer from a test run rather than from reading this paragraph; when it goes red, the condition has fired. [#1054](../../../issues/1054) is the single place this decision lives, and the PR that closes it is the PR that deletes the set.
