...
> **🛑 Retention decision (2026-06-22, issue #258 — do not delete yet; updated 2026-07-02 with #302).** Although the viability gate above is answered, this spike — its route (`/spike/voice-loop`), the three `spike-voice-loop.*` static files, the off-device test, and this doc — is **intentionally kept**. The original criteria were "the kanban / board view **and** the fleet orchestrator (#245) voice mode land": the **board view has now shipped in full** (#164, completed by #302 — including its dictation mics via the shared `voice.js`), so the retention rests **solely on #245**. This loop stays the **live reference** for the #245 follow-ups: wiring real orchestrator narration into the loop in place of the mock, driving narration from summarized board state, and adding a real "conversation mode" entry point. (Note the board's #302 voice work is *dictation-in only* — it reuses none of this loop's narrate/barge-in/re-arm machinery, which is exactly the part #245 still needs.) The `/codebase-audit` stale-finding ([#258](../../../issues/258)) is **deferred on this basis, not unaddressed** — delete the set only once #245's voice mode has shipped and no longer needs it. (As long as it still works, keep it.)
