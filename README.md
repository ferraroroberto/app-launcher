...
- **iPhone regression net** — one focused test per closed iOS-only bite, so the next regression of any of them surfaces locally before a deploy instead of after an hour of phone-PC round-trips:
  ...
  | `test_life_os_tab.py` (`…toggles_live_in_skills_summary_without_options_card`) | (#496) | The Life OS tab grows back a separate options card, the opus/Detached/Resume toggles leave the Skills card's summary, or a toggle tap starts collapsing the panel |
