...
**🕘 Conversations — find one conversation, reopen exactly it** (issue #727). The native picker above is fine for "the thing I was just doing" and useless for "the conversation about the ferry booking, some time in July". A third button on each skill tile opens that skill's **conversation index**: one row per past conversation, newest first, showing the date and the digested topic, expandable to the decisions and open loops that conversation reached. A **🔎 in the Skills card header** opens the same view unscoped — every skill at once, which is the only way to find something when you can't remember which skill it happened in. Typing in the search box runs a **ranked full-text search** across both the digests *and* the full capture text, so an offhand detail no summary would ever mention still finds the right conversation. Each row offers **↺ Resume this** — which reattaches to that exact session (`claude --resume <id>`, honouring the same ☁️ Detached toggle and model combo as any other launch) — plus **📖** to read the raw capture and the existing 🗑️ / ✏️. Ending a resumed conversation *updates* its existing capture and index entry rather than creating a second one, because the capture pipeline identifies a conversation by its opening turn, which survives the session-id rewrite.

This is the one place the launcher's *"never build your own session list"* contract is amended, and only because it isn't building one: the list, the digests, and the resumable session ids are all published by [`life-os`](https://github.com/ferraroroberto/life-os) itself (life-os#68), which writes a `conversations/index.json` beside each skill's `index.md`; the ranking comes from `fleet-config`'s `hooks/conversation_search.py` (fleet-config#586), an SQLite FTS5 index the launcher shells into and never maintains. Both are read defensively: a skill the indexer hasn't digested yet shows an honest empty state, and a missing CLI or database reports **search unavailable** rather than an error. A conversation captured before the session id was stored — roughly a quarter of an established archive — shows a **`no session`** chip: readable, but not reopenable, said out loud rather than hidden behind a disabled button no phone can hover over. Both new endpoints carry the same Tailscale + passkey gate as the Browse viewer, because a digest is a distillation of terminal content and a search hit can quote anything the terminal ever showed.
