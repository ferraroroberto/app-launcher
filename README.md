...
- **iPhone regression net** — one focused test per closed iOS-only bite, so the next regression of any of them surfaces locally before a deploy instead of after an hour of phone-PC round-trips:
  ...
  | `test_hub_readback.py` | (#203, #206) | The `🔊` button's hub voice regresses: `probeHub()` stops caching the `/api/tts/health` verdict, `speakHub()` stops unlocking the `<audio>` element then pointing it at the progressive `/api/tts/stream/{id}` (id staged via `POST /api/tts/speak`, bearer + passkey in the query), a failed stage stops rejecting for Web Speech fallback, or `cancelHub()` stops pausing in-flight audio + resetting the button |
