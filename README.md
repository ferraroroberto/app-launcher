...
- **🎤 Dictate** (#165) — lives **inside the compose bar** (beside ➤ Send), so dictation always goes through review-before-send and never streams raw into the PTY. Tap to start recording the mic, tap again to stop; the recording is transcribed and the text dropped into the textarea at the caret for editing before Send. The phone never talks to the transcriber directly — the webapp proxies the audio over loopback to the sibling [`voice-transcriber`](https://github.com/ferraroroberto/voice-transcriber)'s consumable session API (so the take is saved on the PC, same as a recording made in that app). Gated exactly like the live terminal (Tailscale-only + passkey). Hidden when `voice_transcriber_url` is unset or the browser lacks `MediaRecorder`.
...
- **iPhone regression net** — one focused test per closed iOS-only bite, so the next regression of any of them surfaces locally before a deploy instead of after an hour of phone-PC round-trips:
  ...
  | `test_voice_dictation.py` | (#165) | The `🎤` dictation button leaves the compose bar, or recording stops dropping the `/api/transcribe` transcript into the textarea (so dictation would stream raw into the PTY or vanish) |
