...
| `chief_respawn_enabled` | `true` | Whether the daily fresh-respawn job fires (issue #245). Saving a change resyncs the registered `chief-daily-respawn` job (pause/resume). |
| `chief_respawn_at` | `"05:00"` | Daily fresh-respawn time, `HH:MM` (issue #245). Saving a change re-registers the job's schedule. |
| `chief_worker_cap` | `3` | Max concurrent worker sessions the `/chief` skill may keep running (1-8, issue #245). Read by the skill over loopback via `GET /api/board/chief/settings` — its dispatch rail, phone-tunable. |
...
- **iPhone regression net** — one focused test per closed iOS-only bite, so the next regression of any of them surfaces locally before a deploy instead of after an hour of phone-PC round-trips:
  ...
  | `test_board_chief.py` | (#245) | Chat mode stops routing to the chief (ensure + the `{data, submit:true}` input proxy) and leaks into `/api/board/dispatch`, the chief card loses its distinct crown/tint or its reply stops rendering in the drawer, the chief ✕ loses its confirm (or worker cards gain one), the manual Start affordance disappears when no chief is alive, or the settings dialog stops round-tripping GET → edit → PUT |
