...
- **Life OS** — one tile per skill in your [`life-os`](https://github.com/ferraroroberto/life-os) checkout (the directories under `<life_os_dir>/.claude/skills` whose name doesn't start with `_`, listed live and alphabetically — a new skill folder appears with no restart). Where the Coding tab answers *"run a coding agent in project X"*, this answers *"invoke productivity skill Y, ready for me."*
  ...
  - **Launching a skill** — each tile shows the skill name plus **🚀 Launch** (a fresh Claude session cwd'd in `life-os` that auto-invokes the bare `/skill-name`; no free text is injected, you type into the live terminal once the skill reports ready) and **📖 Browse** (a read-only viewer of what that skill knows about you — full-screen file list, tap to open full-screen, **✕** to close, **🗑️** to delete a disposable conversation log after a confirm). Launch controls live in the Skills card header: a model combo (Sonnet / Opus / Fable), plus icon-only **☁️ Detached** and **↺ Resume** with the same semantics as the Coding tab — Resume drops the `/skill-name` prompt and opens Claude's native session picker via a Remote Control session + `/resume`, so the chosen conversation stays available from Claude mobile/web, and is orthogonal to Detached exactly as on the Coding tab. Every other Claude flag (effort, permission, verbose, debug) comes from the shared **⚙️ Coding options** card. Launched sessions appear in the Coding tab's running-sessions list, re-attachable and killable like any other.
