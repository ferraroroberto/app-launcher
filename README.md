...
- **The dual-projection browser runs use parallel workers (issue #1231).** The `full` and `surface` tiers run on 4 `pytest-xdist` workers (`-n 4 --dist loadgroup`); each worker boots its own disposable webapp + session-host, logs to `webapp/e2e-autoboot-{webapp,session-host}-gw<N>.log`, and the whole set still runs inside the one machine-wide dual-projection mutex (#685), which serialises gates across checkouts. Nothing is dropped: every node still runs on both projections. The count is one setting, `$E2EDefaultWorkers` in `scripts/e2e-gate-route.ps1`, overridable per run with `E2E_WORKERS` (`0` or `1` = serial, as before). The static Chromium smoke stays serial. `tests/e2e/test_terminal_reconnect.py` (the real-agent and reconnect pins) is one `xdist_group`, so its tests never overlap each other.
