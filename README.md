...
- **Coding** — opens with a one-line **summary head card** (issue #496, the fleet's vendored `home-head` component): app icon + title, live stats (`N sessions · M apps running · X dirty · Y off-main`), and the light/dark **theme toggle** pinned right — the same position as the other fleet apps. Below it, every project directory directly under your configured projects folder becomes a tile (no `.code-workspace` or `*-remote.bat` needed — the list is the directory listing, recomputed live; hide folders with a gitignore-style ignore list in Settings). The tile shows the **bare on-disk folder name** and carries **one launch button per coding agent**:
  ...
  - **⋯ Project menu** (issue #977, replacing the #802 VS Code button) — each tile carries one **⋯** anchor between the agent buttons and the GitHub one, the same floating icon + label menu as a running session's ⚙, with three actions:
  ...
  Running sessions are listed above the project tiles, each marked with its agent's icon and tagged `⚡ full control` or `☁️ detached`. Each row carries one **⚙️ gear** (issue #953) that opens a floating menu of the row's actions, a vertical list of icon + label rows (issue #967) — **💬 Transcript** (every full-control row, plus detached Claude and Codex rows — issue #966), **➤ Send message** (detached rows only — see below), **✏️ Rename / link**, and **✕ Stop-and-kill** (issue #253): stop asks the agent to quit cleanly with its own command (`/quit`, Copilot's `/exit`) so its shutdown hooks run, waits briefly for the clean exit, then force-terminates as a fallback — and the window always closes (no confirm; two taps from either place — gear-then-✕ on the list, ⋮-then-Stop in the terminal view). Tap a full-control row to re-attach; in the terminal view tap *‹* to come back, or open the bar's **⋮** menu to stop and kill it right there without going back to the list first. **Transcript** opens the session's whole conversation as a chat-style list that reads on the phone: your typed prompts and the agent's replies are expanded, and everything else — tool calls with their results, thinking, harness plumbing, sub-agent traffic — is folded per run into one line (`3 tool calls · 1 thinking`) you can open, then open item by item — and those groups are **hidden by default** (the bar's 👁 eye toggle shows them, still folded, so the view opens as a plain you ↔ agent exchange). Each prompt/reply card collapses on tap, the bar's ⇕ toggle collapses or expands every turn at once (tool groups stay closed either way), URLs in any text are clickable, and ✕ is always the rightmost control. It reads the agent's own history (Claude Code's session JSONL, or a Codex rollout) in bounded pages: the newest turns load first and **Load older** (or scrolling to the top) pulls in the previous page, so a multi-MB session never gets read whole. Read-only for a full-control session — the terminal stays its input surface — while a detached session that takes Send message gets a composer docked at the bottom of its transcript (issue #975): type and ➤ Send, the same request and *Sent, not confirmed* toast as the menu's dialog, and the newest page reloads a few seconds later so the sent turn shows up (a failed send keeps the text to retry). The phone's floating tab bar hides while the transcript is open, like the terminal view. Served behind the same Tailscale + passkey gate as the terminal. It never needs the launcher's terminal capture, so a detached Claude or Codex session started on the PC reads exactly the same from the phone (a detached row of another agent has no Transcript item). Agents without a history file and missing or unreadable files say so explicitly ("No transcript found" is a different line from "Couldn't read the transcript"). The menu's pencil opens **Rename / link** for Claude and Codex full-control sessions: Claude exposes and copies the provider-native `claude.ai/code/session_…` remote-control URL captured from its terminal, while Codex says **Not available yet** because its local sessions currently have no web URL. Detached sessions and other agents keep the rename-only dialog. A **detached** row's menu also offers **➤ Send message** (issue #967): a small dialog whose Send types the text into the session's console window on the PC and presses Enter, so a long detached run can be steered from the phone without converting it to a full-control session. It is offered only for agents a recorded probe proved take console input this way (Claude Code, Codex, Pi, GitHub Copilot, Antigravity — the registry's `console_input` flag; Grok is not probed, so its detached rows get no item). Delivery is **unconfirmed by design**: a detached console has no output stream the launcher can read, so the API reports `delivered: "unconfirmed"` (never `true`) and the toast says *Sent, not confirmed* — check the console, or the Transcript, if it matters. If the PC's session-host is still running a build from before this feature, the toast says *session-host restart needed* (with the running build's sha) rather than a bare HTTP error. A newline in the message becomes a soft line break in the agent's composer; the one Enter that submits is sent separately after a short settle. The **⚙️ Coding options** card — now the **last card on the tab** (issue #496: configuration moves out of the way of launching; collapsible, collapsed by default) — has a Claude Code subsection (model / effort / permission mode / verbose / debug), an Antigravity subsection (`--dangerously-skip-permissions` / `--sandbox` toggles), and a GitHub Copilot subsection (a `--model` picker plus the `--allow-all` toggle). Antigravity has no launch-time model flag — pick its model with `/model` in-session. See [Interactive terminal](#interactive-terminal-from-the-phone) for the security model.
...
- **Apps** — every `*.bat` under your scan root that the classifier recognises as Streamlit, a FastAPI webapp, or a Cloudflare-tunnel script. Each row carries **two launch buttons** rather than being one big tap target (issue #790): **⚡** runs the bat in a fresh CMD window you can watch, and **🚫👁 stealth** runs the identical command line with **no window on screen** — the phone-first case, where a console popping up on an unattended PC is pure noise. The row body itself is inert, so scrolling past a row can't fire it. Both modes spawn the same way (`cmd /k`, direct child), so a stealth-launched app still binds a port and still appears in **Running apps** with a working Stop. A request that names no mode launches visibly, so an older cached PWA bundle never goes silently invisible. The same pair is on every **Trays** row. Every row is **two lines** — kind pill, then name — with its controls on the right; the bat path is hidden until **Edit mode**, since it wraps to two or three lines on a phone and only matters when you're renaming or removing the row. Tunnel rows put their live URL on a **🔗** in that control cluster rather than as link text (a cloudflared URL carries a `?token=…` and wrapped to three lines); the `href` still holds the whole URL, so tap, copy-link and open-in-new-tab all work, and the row's health dot — refreshed every 4 s — is what reports whether the tunnel is up.
- **Jobs** — one-shot Python scripts and scheduled jobs (`.py` or `.bat` targets), fired from the phone, the Stream Deck, or Task Scheduler through one shared executor with a uniform run history. Every row reads as four fixed lines (name / type+schedule+countdown / duration percentiles + last-7 sparkline / last-run meta) so the same information lands in the same place across jobs.
  ...
  - **Run history** — tap a row to expand recent runs and the most recent output tail; a live selection streams over WebSocket, finalized logs load once. Jobs can preserve downloadable files (`JOB_ARTIFACT_DIR`), and a run can be pinned to survive normal retention. The card search box greps every indexed run output. CPU and peak RSS surface on the selected run; tap the output pane to copy the whole log.
...
- **Life OS** — one tile per skill in your [`life-os`](https://github.com/ferraroroberto/life-os) checkout (the directories under `<life_os_dir>/.claude/skills` whose name doesn't start with `_`, listed live and alphabetically — a new skill folder appears with no restart). Where the Coding tab answers *"run a coding agent in project X"*, this answers *"invoke productivity skill Y, ready for me."*
  ...
  - **Access** — Browse, Conversations, and targeted resume/handoff endpoints are gated harder than the rest of the app: Tailscale-only, refused over the Cloudflare tunnel, passkey-gated (the same gate as the live terminal), and the file-content endpoint is path-jailed to `life_os_dir`. See [Interactive terminal](#interactive-terminal-from-the-phone) for the gate.
...
- **Board** — one screen answering *"what needs me now, across everything"*: a read-only kanban over five **computed** columns, each holding one kind of card (a card moves because reality changed — there is deliberately no drag-and-drop). **Backlog** (open GitHub issues), **Claude's turn** (live sessions working / idle / unknown / finished-clean), **Your turn** (sessions stalled, blocked on a decision, or otherwise waiting on your input — the only column that needs you), **Other** (open PRs plus today's failed or stuck job runs), and **Done** (today's closed issues). On the phone the columns are a swipeable one-column-per-screen carousel with a count strip on top (the *Your turn* count highlights when nonzero); desktop shows all five side by side, each with its own `(N)` header count. See [Board tab](docs/board.md) for the columns' exact data sources, the agent-aware session-state join, and the transcript/conversation-source hierarchy.
  ...
  - **Drill-down drawer** — tapping a live session opens an inline drawer: the last exchange, a full-width reply box, then one right-aligned row of 44px buttons — 🎤 dictate, ➤ send, ✏️ Rename, ✕ Stop (the unified stop path), and ⚡ Terminal last. The conversation preview is agent-aware — structured Claude/Codex history wins when it correlates safely, the launcher's own PTY capture is the fallback — and all parsing happens on the drawer request, never the 5 s poll.
...
A small bar sits above the terminal for the things a phone keyboard can't do well, and the **composer** is always docked under it (issue #980, Step 1 of #979 — the session-parity project that gives every session view the same composer and a five-control bar). The composer keeps one shape everywhere: a tall predictive `<textarea>` plus a 2×2 button grid — **🎤 mic · ⌨️ keys** on the first row, **🖼 image · ➤ send** on the second. There is no Compose toggle any more: xterm.js wipes its own helper textarea on every keystroke, so iOS/Android autocomplete can't suggest there, while the composer is a plain textarea, so it can. Type (or dictate, or attach), then **➤ Send** forwards the buffered text to the PTY and the submitting `Enter` as a separate frame (#166). Paste goes into the textarea like any text field — the old bar buttons for Paste, Image, Compose and Keys are gone, folded into the composer. A mic or keys button that can't work here (no voice-transcriber configured; no PTY behind the session) stays in the grid **disabled** rather than hidden, so the grid never changes shape. The composer is rendered by one module, `composer.js`, with a single mount entry point; the terminal mounts it from `terminal-compose.js`, and the Chat mode and Board drawer steps of #979 mount the same thing. Hidden in the PC mirror window (#37), which has a real keyboard.

- **⋮ Session menu** (#981, Step 2 of #979) — the bar's last control. It is the bar's whole session-management surface, so the bar stays at five controls on a 390px phone (‹ Back · title · mode toggle · 🔊 · ⋮) without scrolling sideways: **Rename** opens the same Rename / link dialog as the sessions list's gear and updates the bar title as soon as you save; **Copy link** copies this launcher's own `?session=<sid>` link, which opens straight into this session on any device signed in to the launcher (unlike the PC mirror's `?terminal=` link, it never takes over the phone's terminal size); **Stop and kill** is the unified stop path (#253), the same as the list's ✕ — no confirm, except for the fleet chief. It replaced the permanent in-bar ✕.
- **↓ Latest** (#981) — replaces the bar's old ↓ Jump button. A small pill floats at the bottom-right of the terminal, just above the composer, but only while you are scrolled more than one screen above the newest output; tap it to jump back down and it hides again. At the tail, or a few lines up, it stays out of the way.
- **⌨️ Keys** — the composer grid's second button opens a popover D-pad **above the composer**, where the thumb reaches it, of arrow / `Esc` / `Tab` / `Enter` keys for iPhone keyboards (SwiftKey etc.) that lack them, so Claude's TUI prompts stay navigable (#36). Disabled when the session has no PTY to drive. Includes a sticky **`⇧` Shift** toggle (#137): tap it to hold Shift (it lights up), then `Tab` sends **Shift+Tab** — how Claude Code cycles permission modes (auto-accept edits → plan → dangerous-skip-permissions). It stays held across taps so you can chain the cycle; tap it again or close the popover to release.
...
│   └── jobs*.py, jobs_kinds/   # Jobs backend (17 modules) — config chain, scheduling/trigger/queue/reap,
...
The same gate also runs on CI (`.github/workflows/e2e.yml`, `windows-2025`) on every push to `main` and on pull requests into `main` — so the gate runs without relying on remembering to. The local `verify-before-ship.ps1` stays the contract; CI is supplementary.
