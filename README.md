...
- **⋮ Session menu** (#981, Step 2 of #979) — the bar's last control. It is the bar's whole session-management surface, so the bar stays at five controls on a 390px phone (‹ Back · title · mode toggle · 🔊 · ⋮) without scrolling sideways: **Rename** opens the same Rename / link dialog as the Board drawer's Rename button and updates the bar title as soon as you save; **Copy link** copies this launcher's own `?session=<sid>` link, which opens straight into this session on any device signed in to the launcher (unlike the PC mirror's `?terminal=` link, it never takes over the phone's terminal size); **Stop and kill** is the unified stop path (#253) — no confirm, except for the fleet chief. It replaced the permanent in-bar ✕.
