...
- **🔊 Read aloud** (#190) — a top-bar control **between ↓ Jump and 📋 Paste** (not in the compose bar — that's for editing), the eyes-free other half of dictation for driving / walking. Tap to hear the agent's **last reply** spoken back — the final answer or the question it's asking you — so you can keep the phone in your pocket: dictate → send → 🔊 → listen → dictate again. It reads **only the last reply**, not the chrome around it: the text is lifted client-side from the xterm scrollback, which on the phone is a raw TUI of redraws, boxes and a live status footer. The extractor anchors on the input composer box and **drops everything from it down** — the box, the folder/branch line, the permission-mode line, the token count and the spinner — then **skips the `recap:` block, the per-turn timing line** (`✻ Crunched for 5s …` / `Worked for 21m 17s`) **and the live thinking spinner** (`✻ Cogitating… (4m 39s · thinking)` — even the form with no token count, #193) — all three matched by shape, not verb, since Claude Code picks a random gerund — and reads the last prose turn above the previous tool/user boundary, de-wrapped from the phone's 51-column wraps into one paragraph, with the leading `●` assistant turn-marker stripped. It also **discards the spinner's `⎿ Tip:` hint** (`⎿ Tip: Running multiple Claude sessions? Use /color and /rename …`, #195) — that help line renders as a tool-result child whose wrapped continuation carries no `⎿` glyph, so it would otherwise be read as the reply; instead the real turn *above* the spinner is read. If the agent is mid-work with no settled reply above the spinner (a tool boundary directly above it) it says nothing rather than read the status line. When the reply finishes reading it resets the button and pops a **🔊 Finished reading** toast (with a watchdog backstop because iOS fires the speech-`end` event unreliably). Speaking uses the browser's built-in **Web Speech API** (`speechSynthesis`) — on-device, zero server, the iOS Siri-enhanced voices when installed; a richer server-side hub voice ([local-llm-hub#98](https://github.com/ferraroroberto/local-llm-hub/issues/98)) is the documented phase-2 fallback. Tap again (or starting a new dictation, or leaving the tab) stops the read-aloud. Hidden where the browser can't synthesize speech, like 🎤 is hidden without `MediaRecorder`.
...
- **iPhone regression net** — one focused test per closed iOS-only bite, so the next regression of any of them surfaces locally before a deploy instead of after an hour of phone-PC round-trips:
  ...
  | `test_voice_readback.py` | (#190) | The `🔊` read-aloud button leaves the terminal toolbar (it must sit between ↓ Jump and 📋 Paste, not in the compose bar); the last-reply extractor stops skipping the composer box + status footer (folder / permission / tokens) or the `recap:` / `Worked for …` lines, stops de-wrapping the reply, or stops staying silent mid-work; `speak()` stops queuing per-sentence utterances or `cancelSpeech()` stops them; or starting a new dictation stops silencing the in-flight read-aloud |
