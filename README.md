...
- **Board** — one screen answering *"what needs me now, across everything"*: a read-only kanban over five **computed** columns, each holding one kind of card (a card moves because reality changed — there is deliberately no drag-and-drop). **Backlog** (open GitHub issues), **Claude's turn** (live sessions working / idle / unknown / finished-clean), **Your turn** (sessions stalled, blocked on a decision, or otherwise waiting on your input — the only column that needs you), **Other** (open PRs plus today's failed or stuck job runs), and **Done** (today's closed issues). Like every other tab, the Board is built from collapsible section cards (#1198): a **Dispatch** card first, then one card per column with its count in the header (the *Your turn* count highlights when nonzero). On the phone they stack, with the two live columns open and the GitHub-fed ones folded; desktop lays all five side by side, open. Every card leads with its title (two lines at most) and the repo and number under it. Directly under *Claude's turn*, in both layouts, sits the **Chief's plan** card (issue #1279): what the fleet chief is doing and intends to do next, read from the plan file it keeps (`chief_plan_file`, format v1, written by fleet-config's `chief_ops.py plan`). It lists what is waiting on you first, then the chief's lanes (repo, current item, status), then its queue in order, each row with a status chip (an unknown status gets a neutral one), and says when the plan was last updated. When no chief session is running it says *Chief not running*, and *Chief status unknown* when the session list can't be read, so an old plan never reads as current. With no plan the card stays blank; a plan it can't read adds a quiet *Plan unreadable* note and no error. It refreshes with the Board's own poll. See [Board tab](docs/board.md) for the columns' exact data sources, the agent-aware session-state join, and the transcript/conversation-source hierarchy.
...
Launching a Coding-tab project in **full control** mode (the default — the ☁️ Detached toggle off) opens a **live terminal** — the same thing you'd see in the CMD window on the PC, streamed to the phone: real output, scrollback, typing, `Ctrl+C`, `/quit`, and image paste. This works the same for any coding agent (Claude Code, Codex CLI, Antigravity CLI, GitHub Copilot CLI, Pi, or Grok Build). Tap a `⚡ full control` session in the list to re-attach. The terminal is the **Terminal mode** of the session view (issue #982): the bar's Terminal ⇄ Chat toggle switches to **Chat mode** (issue #953 — the whole conversation, prompts and replies expanded, tool calls folded; see the Coding-tab overview above) and back without reconnecting the terminal, and the ⋮ menu's **Terminal** / **Chat** items open the view straight into either mode. Just before the toggle, a small **context ring** (issue #1223) shows how full a full-control Claude session's context window is: its border fills with the percentage (accent, then amber from 60% and red from 80%), and a tap toasts the number. The value is Claude Code's own, read off the terminal's statusline (the fleet `statusline-command.ps1` footer's `NN%c`) by `GET /api/claude-code/sessions/{sid}/context`, polled every 10 s in both modes while the view is open and the page visible. The ring is simply absent, never a 0% guess, when the number isn't known: a detached or non-Claude session, a capture that can't be read, or no `%c` on screen (Claude Code reports none early in a session and right after `/compact`). If the statusline's format changes, the ring goes blank rather than wrong. **Terminal scrollback is cleared whenever Claude Code repaints the whole screen** (issue #930): once one turn grows taller than the terminal, Claude redraws the full viewport, and that redraw used to land a second copy of what was already in scrollback. Now the older scrollback goes instead. Scroll back in Terminal for recent output. To read the whole conversation, use Chat, which rebuilds it from the agent's own history and never duplicates. Its ✏️ action opens **Rename / link**: below the editable launcher title, a read-only URL and link button copy the session's **provider-native** link — for Claude Code the `claude.ai/code/session_…` remote-control URL captured from its own terminal output, which is authenticated by the Anthropic account and so opens from any browser on any network (Codex sessions show **Not available yet**: its local sessions have no web URL). Detached sessions keep the same rename action but do not show a link because they have no streamable PTY; they are steered from Chat mode's composer instead (issues #967/#983), which types a follow-up into the PC console — delivery unconfirmed, see the Coding-tab overview above.
...
| `chief_plan_file` | `~/.claude/hooks/state/chief-plan.json` | The fleet chief's plan (format v1, written by fleet-config's `chief_ops.py plan`), shown by the Board's **Chief's plan** card. Absent or an empty queue leaves the card blank; unreadable JSON or an unsupported `version` adds a quiet *Plan unreadable* note, never an error. |
...
- **iPhone regression net** — one focused test per closed iOS-only bite, so the next regression of any of them surfaces locally before a deploy instead of after an hour of phone-PC round-trips. `tests/e2e/` is its own index — don't re-enumerate the closed bites here; each regression test's docstring names the issue it pins and what would regress without it (e.g. `tests/e2e/test_paste_framing.py:1`, `tests/e2e/test_primary_nav.py:1`).

Every test runs on **Chromium-desktop**, and the ones where the engine or the phone shape matters also run on **WebKit projected onto an iPhone 15 Pro Max viewport** (#1220): layout and geometry, touch targets, nav, the composer and keyboard input, safe-area, and anything that branches on the engine. That set carries the `iphone` pytest marker, and `tests/e2e/conftest.py` deselects every other test's WebKit node at collection. `tests/test_e2e_iphone_projection.py` fails the build when a test that measures layout, taps or reads `browser_name` is missing the marker. Real iOS-only bugs don't reproduce in desktop WebKit, so the on-device check stays the final word. Pin a single engine with `--browser chromium` (or `webkit`) for a faster dev loop.
...
- **Data:** synthetic only. A demo project, one stub PTY session (the e2e gate's `--e2e-stub` child, from `tests/e2e/stub_session.py`) with a synthetic Chat transcript, one Life OS skill, one job and one app. Every config path, `USERPROFILE`/`HOME` and `CLAUDE_CONFIG_DIR` point into the temp tree. The Board's GitHub lists come from a fake `gh` (`scripts/synthetic_gh.py`, run through `LAUNCHER_GH_CMD`) that returns a few synthetic issues and one PR, never the real CLI, so no real issue title reaches a walk's screenshots or report (#1286).
...
- **Surfaces narrow a `full` diff to one tab (issue #955).** `.fleet.toml` declares `[[e2e.surface]]` entries — today **board** (`board.js`, `board-dispatch.js`, the `board*` routers, their e2e tests) and **lifeos** (`life-os.js`, the `life_os*` routers, their e2e tests). A diff whose every browser-surface path sits in one surface runs only that surface's `pytest_targets`, still on both projections and still under the machine-wide e2e mutex: the tab's own tests, the tests that drive its modules from elsewhere, plus `test_smoke.py` and `test_primary_nav.py` (which carries the bottom tab bar's checks), since a broken tab module takes the whole SPA down. Shared files (`styles.css`, `index.html`, `main.js` and the other shared JS, `server.py`, the session-host paths, both conftests) belong to no surface. A diff touching one of those, spanning two surfaces, carrying an unclassified path, or editing `.fleet.toml` / the classifier runs the whole suite. `tests/test_classify_e2e.py` pins each surface and fails when an e2e test that names a surface's modules is missing from its targets.
- **The dual-projection browser runs use parallel workers (issue #1231).** The `full` and `surface` tiers run on 2 `pytest-xdist` workers (`-n 2 --dist load`); each worker boots its own disposable webapp + session-host and logs to `webapp/e2e-autoboot-{webapp,session-host}-gw<N>.log`. Tests marked `serial` (`tests/e2e/test_terminal_reconnect.py`: the real-agent replay pin and the reconnect pins, which failed under the other workers' load) are left out of that pass and run in a short serial pass after it. The whole browser phase still runs inside the one machine-wide dual-projection mutex (#685), which serialises gates across checkouts. Nothing is dropped: every node still runs on both projections. The count is one setting, `$E2EDefaultWorkers` in `scripts/e2e-gate-route.ps1`, overridable per run with `E2E_WORKERS` (`0` or `1` = serial, as before). The default is 2 rather than more because the workers share this box with the live webapp: at 3 or 4 its `/api/board` answered in about 10 s for the whole run, against 0.3 s serially and 3 s at 2 (the measured table is in #1231's PR). `E2E_WORKERS=4` is faster when nobody is using the live app. The static Chromium smoke stays serial. Each test also gets back any server-side setting it changed (`tests/e2e/conftest.py::_restore_disposable_config`), so no test depends on a sibling running after it on the same worker.
