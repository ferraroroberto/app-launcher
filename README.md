...
- **Coding** — opens with a one-line **summary head card** (issue #496, the fleet's vendored `home-head` component): app icon + title, live stats (`N sessions · M apps running · X dirty · Y off-main`), and the light/dark **theme toggle** pinned right — the same position as the other fleet apps. Below it, every project directory directly under your configured projects folder becomes a tile (no `.code-workspace` or `*-remote.bat` needed — the list is the directory listing, recomputed live; hide folders with a gitignore-style ignore list in Settings). The tile shows the **bare on-disk folder name** and carries **one launch button per coding agent**:
  ...
  - **Open in VS Code** (issue #802) — each tile carries a **Visual Studio Code** button between the agent buttons and the GitHub one: one tap opens that project in the local editor, for when you want to eyeball a diff or browse files in a real editor rather than a terminal. It opens the project's `<name>.code-workspace` file — a **sibling** of the project folder, directly under `projects_dir`, the layout VS Code itself already uses here — and **creates that file first** with a minimal one-folder shape when it doesn't exist yet; an existing workspace file is opened, never rewritten. Deliberately *not* a coding-agent launch: no PTY, no session, nothing in Running sessions and no Stop button — VS Code is its own top-level app once it's up. Like the agent buttons it greys out with a hover hint when the `code` CLI isn't on PATH (VS Code installs it through its own *Shell Command: Install 'code' command in PATH* step), and it's hideable from the same **Visible agents** list below. A **local-machine** action, like every Coding-tab launch: the editor opens on the PC the launcher runs on, not on the phone that tapped it.
...
| `coding_hidden_agents` | `[]` | Coding-row launch buttons hidden from the project rows (issue #666) — agent ids plus the pseudo-ids `vscode` and `github`. Managed by the **Visible agents** switches in the ⚙️ Coding options card. A *hidden* list, so a newly registered agent appears by default. |
