...
- **⋮ Session menu** (#981, Step 2 of #979) — the bar's last control. It is the bar's whole session-management surface, so the bar stays at five controls on a 390px phone (‹ Back · title · mode toggle · 🔊 · ⋮) without scrolling sideways: **Rename** opens the same Rename / link dialog as the sessions list's ⋮ kebab and updates the bar title as soon as you save; **Copy link** copies this launcher's own `?session=<sid>` link, which opens straight into this session on any device signed in to the launcher (unlike the PC mirror's `?terminal=` link, it never takes over the phone's terminal size); **Stop and kill** is the unified stop path (#253) — no confirm, except for the fleet chief. It replaced the permanent in-bar ✕.
...
Since the #534 fixture split, the UI-only terminal tests run on CI too: their lightweight stub child needs only Python, so the CI runner exercises them instead of skipping. Only the `launched_claude_pty_session` tests (real-Claude rendered-output assertions) still check `claude` is on `PATH` and **skip** cleanly where it isn't — notably the CI runner, which never installs it (issue #58). A failed run keeps the autoboot logs and `verify-progress.log` as a downloadable `e2e-logs` artifact on the run page, so any e2e failure can be diagnosed without a local repro. It carries no per-session logs: #913 gives the disposable processes a per-run temp audit dir so a gate run never writes into `webapp/sessions/`, which is enforced by the harness's own isolation check (#1004).
