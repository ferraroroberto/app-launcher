...
- **Board** (issue #164, complete: #300 + #301 + #302) — one screen answering *"what needs me now, across everything"*: a read-only kanban over four **computed** columns (a card moves because reality changed — there is deliberately no drag-and-drop). **Backlog** = open GitHub issues across every repo of the configured owner; **Claude's turn** = live coding sessions that are working / idle / unknown; **Your turn** = sessions waiting on your input, open PRs, and today's failed or stuck job runs; **Done** = today's merged PRs + closed issues. On the phone the columns are a swipeable one-column-per-screen carousel with a count strip on top (the *Your turn* count is the number that matters — it highlights when nonzero); desktop shows all four side by side. Three data sources, each degrading independently: the live session list from the session-host; the **sessions-state file** written by [`fleet-config`](https://github.com/ferraroroberto/fleet-config)'s `session_state` hook (fleet-config#91 — `working` on your prompt, `needs-you` when Claude stops or blocks; joined to live sessions by **normalized cwd**, since hook session ids are transcript UUIDs that never match session-host ids; absent/stale file → sessions show `unknown` rather than erroring); and **GitHub via `gh`** — fetched server-side and cached, refreshed **only** by the ↻ button or automatically on tab open when the cache is older than ~2 minutes, never on the 5 s poll (which just reads the cache), exactly like the ⎇ git-status button's on-demand contract. Tapping a live session card opens an **inline drill-down drawer** (issue #301): the last user↔assistant exchange read from the session's transcript (Tailscale + passkey gated — it is terminal-grade content), a **reply box that types straight into the live PTY** (bracketed-paste framed, CR delivered as its own write per the #166 rule) and a **⚡** into the full-control terminal; replying closes the drawer and the card flips to *Claude's turn* within a poll. While a drawer is open the 5 s poll pauses, so a re-render can never wipe a half-typed reply. Backlog cards whose repo is present in the projects folder carry **▶ Start / ⚡ YOLO** — one tap spawns a Claude session cwd'd in that repo running the server-built `/issue-start <N>` / `/issue-yolo <N>` (client text never reaches the command line), inheriting worktree isolation for free — the `/issue-*` skills claim the repo and build in a sibling worktree when the primary checkout is busy. `?board=<sid>` deep-links onto a session's card with its drawer open (the Slack-ping landing page). Issue/PR/done cards open GitHub; a job card jumps to the Jobs tab. Status accuracy is defended by the **transcript-activity overlay** (issue #305): a `needs-you`/`idle` hook stamp is overridden to `working` when the transcript shows a **real conversation line** (assistant/user message, probed from a bounded tail read) newer than the stamp — so a session resumed via an answered question or a queued prompt (which fire no hook) reads correctly, while the metadata lines Claude Code appends *after* a turn ends (PR links, title refreshes, file-history snapshots) don't fake a resume (issue #309, which is why raw file mtime is only a pre-filter). Pinned above the columns, the **dispatch bar** (issue #302) speaks a new goal into existence: a goal box, a repo `<select>` (the same live claude-code listing the Coding tab launches from), an **add / build / yolo** mode pill (→ `/issue-add`, `/issue-add now`, `/issue-yolo`) plus an **opus** toggle (Life OS semantics: off = Sonnet, on = Opus), a 🎤 and ➤. Dispatch is **injection-safe by spawn-then-type**: the server spawns the Claude session with *no* positional prompt, waits until the agent has painted its first output (the session-host's `output_chars` signal, with a fixed-grace fallback for a host that predates it), then *types* `/issue-<mode> <goal>` into the PTY inside bracketed-paste framing with the submitting CR as its own second write — the free-text goal never touches the unquoted spawn command line, and a session that never becomes ready is killed rather than stranded. PTY-only (a detached console has no input path). The bar keeps its text after a send so rapid multi-dispatch works (✕ clears), and the new card lands in *Claude's turn* on the next poll. Voice dictation — the compose bar's exact streamed-partials pipeline, extracted to a shared `voice.js` — mounts on the dispatch goal box **and** on every drawer reply box, so both "speak a goal" and "speak a reply to a waiting session" are one mic tap; the transcript always lands in the box for review, never straight into a dispatch.
...
- **iPhone regression net** — one focused test per closed iOS-only bite, so the next regression of any of them surfaces locally before a deploy instead of after an hour of phone-PC round-trips:
  ...
  | `test_session_title_naming.py` | (#266) | `sessionTitle()` precedence regresses — a real Claude summary stops winning, a `<folder>`-echo title (Codex/Pi) stops yielding to the first-prompt title, or an agent with no title stops falling back to it; or `mirrorDocTitle()` drops the human title or the `app-launcher-mirror-<sid>` close marker |
