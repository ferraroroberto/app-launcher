...
- **Coding** — opens with a one-line **summary head card** (issue #496, the fleet's vendored `home-head` component): app icon + title, live stats (`N sessions · M apps running · X dirty · Y off-main`), and the light/dark **theme toggle** pinned right — the same position as the other fleet apps. Below it, every project directory directly under your configured projects folder becomes a tile (no `.code-workspace` or `*-remote.bat` needed — the list is the directory listing, recomputed live; hide folders with a gitignore-style ignore list in Settings). The tile shows the **bare on-disk folder name** and carries **one launch button per coding agent**:
  ...
  - A **launch-model selector** sits in the Projects header beside those toggles. It offers provider-qualified **Claude · Sonnet/Opus/Fable** and **Codex · Luna/Terra/Sol/Astra** choices, remembers each provider's default, and sends the selected value when its matching agent button is tapped. The options card exposes the same server-owned catalog and model-compatible effort choices.

  - The Running sessions header shows the selected route's native quota windows from fleet-config's shared snapshot contract. Switching Claude/Codex clears the previous provider immediately; measured `0%`, stale evidence, unknown evidence, unsupported sources, and collector errors remain distinct. Codex refreshes through fleet-config's bounded native one-shot collector when its cached observation expires. Pi/Grok stay unknown until an adapter verifies their actual provider account; a shared-account label appears only when the snapshot carries a verified common pool.
...
- **Life OS** — one tile per skill in your [`life-os`](https://github.com/ferraroroberto/life-os) checkout (the directories under `<life_os_dir>/.claude/skills` whose name doesn't start with `_`, listed live and alphabetically — a new skill folder appears with no restart). Where the Coding tab answers *"run a coding agent in project X"*, this answers *"invoke productivity skill Y, ready for me."*
  ...
  - **Launching a skill** — each tile shows **🚀 Launch** and **📖 Browse**. The Skills header mirrors the Coding selector: Claude Sonnet/Opus/Fable invokes the native slash command, while Codex Luna/Terra/Sol/Astra receives an explicit instruction to load the same project skill from its `.claude/skills/<name>/SKILL.md`. Stored conversation IDs remain provider-specific, so the history Resume selector offers Claude models for the Claude conversations the Life OS index currently publishes.
  - **🕘 Conversations** — a third tile button opens that skill's conversation index (newest first, date + digested topic, expandable to the decisions and open loops it reached); a **🔎** in the Skills card header opens the same view unscoped across every skill, with a ranked full-text search across both digests and full capture text. Each row offers **↺ Resume this** (reattaches to that exact session, honouring the same Detached toggle and model combo), **📖** to read the raw capture, and the existing 🗑️ / ✏️. Ending a resumed conversation updates its existing capture and index entry rather than creating a second one — the capture pipeline identifies a conversation by its opening turn, which survives the session-id rewrite. This is the one place the launcher's *"never build your own session list"* contract is amended, and only because it isn't building one: the list, digests, and resumable session ids are published by `life-os` itself (writing `conversations/index.json` beside each skill's `index.md`); the search ranking comes from `fleet-config`'s SQLite FTS5 index, which the launcher only shells into and never maintains. Both are read defensively — an undigested skill shows an honest empty state, a missing CLI or database reports **search unavailable**, and a conversation captured before session ids existed shows a **`no session`** chip (readable, not reopenable).
  - **Access** — the Browse and Conversations endpoints are gated harder than the rest of the app: Tailscale-only, refused over the Cloudflare tunnel, passkey-gated (the same gate as the live terminal), and the file-content endpoint is path-jailed to `life_os_dir`. Read-only in v1. See [Interactive terminal](#interactive-terminal-from-the-phone) for the gate.
...
- **Board** — one screen answering *"what needs me now, across everything"*: a read-only kanban over five **computed** columns, each holding one kind of card (a card moves because reality changed — there is deliberately no drag-and-drop). **Backlog** (open GitHub issues), **Claude's turn** (live sessions working / idle / unknown / finished-clean), **Your turn** (sessions stalled, blocked on a decision, or otherwise waiting on your input — the only column that needs you), **Other** (open PRs plus today's failed or stuck job runs), and **Done** (today's closed issues). On the phone the columns are a swipeable one-column-per-screen carousel with a count strip on top (the *Your turn* count highlights when nonzero); desktop shows all five side by side, each with its own `(N)` header count. See [Board tab](docs/board.md) for the columns' exact data sources, the agent-aware session-state join, and the transcript/conversation-source hierarchy.
  ...
  - **Git-state colour & in-progress marker** — Backlog cards are colour-coded from the same client-side git-status cache as the Coding tiles (red = dirty tree, yellow = off its default branch), never from the board's 5 s poll. A separate `active-issues.json` lifecycle marker (written by `fleet-config`'s issue workflows) tints a card already in flight, labels it "in progress", and disables both Start/YOLO actions; a missing, corrupt, or stale (>24h) marker degrades to an ordinary actionable card.
  ...
  - The dispatch model selector also selects the Board quota row. Its native provider buckets wrap on phone layouts, retain their own duration/reset labels, and use the browser's local timezone for UTC reset timestamps.
...
│   └── jobs*.py, jobs_kinds/   # Jobs backend (16 modules) — config chain, scheduling/trigger/queue/reap,
│                               #   history/stats/coverage, webhooks, secrets, per-kind executors
