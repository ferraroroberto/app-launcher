...
def jobs_attention(*, now: Optional[datetime] = None) -> List[Dict[str, Any]]:
    """Failed-today, unconfirmed-today and stuck runs across all registered jobs.
    ...
    or ``"stuck"``. ``error`` carries the exit code's own one-liner when it has
    one, so the card can say *why* without a log dive.
